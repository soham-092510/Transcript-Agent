      ...
      <div className="w-full max-w-lg rounded-2xl glass-panel border border-white/10 shadow-2xl overflow-hidden animate-in fade-in zoom-in-95 duration-150 text-slate-200">
        ...
        <div className="px-6 py-4 border-b border-white/10 flex items-center justify-between bg-white/5">
          <div className="flex items-center gap-2.5">
            <div className="w-8 h-8 rounded-lg bg-cyan-500/15 border border-cyan-400/30 text-[#00D9FF] flex items-center justify-center font-bold shadow-neon-blue">
              <SettingsIcon className="w-4 h-4" />
            </div>
            <div>
              <h3 className="font-bold text-[#F0F9FF] text-sm">System & AI Engine Preferences</h3>
        ...
        <div className="p-6 space-y-5 text-xs text-slate-300">
          ...
          <div className={`p-4 rounded-xl border transition-all ${
            fastMode 
              ? 'bg-cyan-500/10 border-cyan-400/40 ring-1 ring-cyan-500/30' 
              : 'bg-white/5 border-white/10'
          }`}>
            <div className="flex items-start justify-between">
              <div className="space-y-1">
                <div className="flex items-center gap-1.5 font-bold text-[#F0F9FF]">
                  <Zap className="w-4 h-4 text-[#00D9FF] fill-[#00D9FF]" />
                  <span>Zero-Lag Grounded Mode (Recommended)</span>
                  <span className="px-2 py-0.5 rounded-full bg-cyan-500/20 text-[#00D9FF] border border-cyan-400/30 text-[10px] font-mono font-bold">
                    0.0s Lag
                  </span>
                </div>
              ...
              <input
                ...
                className="mt-1 w-5 h-5 rounded text-[#00D9FF] focus:ring-cyan-500 cursor-pointer accent-[#00D9FF]"
              />
            </div>
          </div>
          ...
          <div className="p-4 rounded-xl bg-white/5 border border-white/10 space-y-3">
            ...
            {installedModels.length > 0 ? (
              <select
                ...
                className="w-full glass-input rounded-lg px-3 py-2 text-xs font-mono font-semibold text-[#F0F9FF] focus:outline-none focus:border-[#00D9FF]"
              >
                {installedModels.map((m) => (
                  <option key={m} value={m} className="bg-[#12122B] text-slate-200">{m}</option>
                ))}
              </select>
            ) : (
        ...
        {/* Footer */}
        <div className="px-6 py-3.5 border-t border-white/10 bg-transparent flex items-center justify-between">
          <button
            onClick={loadSettings}
            disabled={isLoading}
            className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-slate-400 hover:text-slate-200 hover:bg-white/5 text-xs font-semibold transition-colors"
          >
          ...
          <div className="flex items-center gap-2">
            <button
              onClick={onClose}
              className="px-3.5 py-1.5 rounded-lg border border-white/10 hover:bg-white/10 text-slate-300 text-xs font-semibold transition-colors"
            >
              Cancel
            </button>
            <button
              onClick={handleSave}
              disabled={isLoading}
              className={`flex items-center gap-1.5 px-4 py-1.5 rounded-lg text-white text-xs font-semibold shadow-neon-blue transition-all ${
                savedSuccess ? 'bg-emerald-600' : 'neon-glow-btn'
              }`}
            >
              {savedSuccess ? (
                <>
