...
import { Loader2, Sparkles, Radio, RefreshCw, AlertCircle, GraduationCap, Zap, Server, Globe } from 'lucide-react';
...
import { api, getApiBase, setBackendUrl } from './services/api';
    ...
    <div className="flex h-screen w-screen overflow-hidden bg-[#0A0A1F] techno-grid-bg font-sans text-[#E2E8F0]">
      ...
      <div className="flex-1 flex flex-col h-full overflow-hidden">
        ...
        {activeSession ? (
          ...
          <div className="flex-1 flex flex-col items-center justify-center p-8 text-center bg-transparent max-w-xl mx-auto">
            ...
            <div className="mt-4 px-3 py-1.5 rounded-lg bg-slate-900/80 border border-slate-700 text-[11px] text-slate-300 font-mono flex items-center gap-2">
              <span className="text-slate-400">Target Endpoint:</span>
              <span className="text-sky-300 font-semibold">{getApiBase()}</span>
            </div>

            <div className="mt-6 flex flex-wrap items-center justify-center gap-3">
              <button
                onClick={() => loadInitialData(5, 1200)}
              ...
              >
                ...
                {showBackendConfig ? 'Hide Config' : 'Configure Backend URL'}
              </button>
              <button
            ...
            {showBackendConfig && (
              <div className="mt-6 w-full p-4 rounded-2xl bg-slate-900/90 border border-slate-700/80 text-left space-y-3 animate-in fade-in duration-200">
                <div className="text-xs font-semibold text-slate-200">Backend API URL Configuration</div>
                <p className="text-[11px] text-slate-400">
                  Override the target backend server URL if your backend is hosted on a custom Render URL or local port.
                </p>
                <div className="flex gap-2">
                  <input
                    ...
                    placeholder="https://transcript-agent-backend.onrender.com"
                    className="flex-1 px-3 py-1.5 rounded-lg bg-slate-800 border border-slate-700 text-xs text-slate-100 placeholder-slate-500 focus:outline-none focus:border-sky-500"
                  />
                  <button
                    onClick={() => {
                      if (customBackendInput.trim()) {
                        setBackendUrl(customBackendInput.trim());
                ...
                <div className="flex flex-wrap gap-2 pt-1 text-[11px]">
                  <button
                    ...
                    className="px-2.5 py-1 rounded-md bg-slate-800 hover:bg-slate-700 text-slate-300 border border-slate-700 cursor-pointer"
                  >
